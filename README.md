...
13. 提示词交付前缺少机械检查，容易遗漏 5000 字符、0:00 起始、15 秒上限等硬规则。
...
21. 实景故事板污染 Seedance 参考，导致人物、服装、场景纹理被错误复制。
...
**Seedance prompt 写"图画不出来的东西"（用满 5000 字符上限）**：
...
核心升级：**180度轴线锁 + B-roll/反应镜头设计**。对话、对峙、行动和离开场景必须在故事板顶视图里画出180度轴线、可用机位半区、禁入机位半区、眼线和入口/出口方向；Seedance 提示词必须逐镜头写清固定左右关系，禁止越轴、眼线反转、地理翻转和离开方向反转。同时不再把文戏只写成连续正反打，必须按剧情需要加入有功能的 B-roll、反应镜头、插入镜头和空境，用于道具连续性、情绪呼吸、空间重置和声音节奏。完整模板详见 [提示词武器库索引](references/prompt-library-index.md)。
