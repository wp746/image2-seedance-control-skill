...
核心升级：**VOICE_LOCK 角色音色锁**。主要和反复出场的说话角色必须在 Seedance 提示词里建立固定音色：年龄声线、音高、音质、气息、音量、语速、咬字、口音强度、情绪下的声音变化和禁止错误声音。每句台词前必须调用对应角色的音色锁和当下表演状态，台词原文仍逐字保留。上一版的 180 度轴线锁、B-roll/反应镜头设计继续保留。完整模板详见 [提示词武器库索引](references/prompt-library-index.md)。
