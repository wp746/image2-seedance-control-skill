...
- `realism-clean-frame-rhythm-gate.md`: hard gate for realist war/historical/documentary live-action projects: photographic medium lock, Simplified Chinese label rule, text-clean frame boundary, GBK/GB18030 intake warning, and 15-second rhythm/breathing shot-count rules.
...
- `scene-type-playbook.md`: scene-type routing playbook — 6 dramatic functions (DIALOGUE/ACTION/SUSPENSE/TRANSITION/PRODUCT/ATMOSPHERE) each with dedicated storyboard design + Seedance prompt strategy + camera language + sound design + performance direction + taboos. A-roll vs B-roll design principles. Auto-detection from script/user input triggers.
...
- `film-industry-master-checklist.md`: ultimate quality benchmark covering 10 filmmaking departments — Producing (model capabilities), Production Design (color system/spatial psychology/narrative objects/material language), Cinematography (aspect ratio/lens psychology/depth of field/composition grammar/camera movement language/film stock color science), Lighting (quality/position/ratio/color temperature motivation), Sound (room tone/ambience/foley/dialogue layers, narrative function), Performance (5 levels from action to breath, reaction chains), Editing (cut types/rhythm/180-degree rule), HMU (wardrobe/hair/makeup as narrative), VFX (practical-first, AI-flavor avoidance), Rhythm & Breathing (scene-level/segment-level/segment-to-segment/sound rhythm). 40+ item master checklist at document end.
