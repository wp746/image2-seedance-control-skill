...
- Dialogue, confrontation, pursuit, and exit scenes must include a **180-degree axis / screen-direction lock**. The storyboard top plan must draw the action axis, safe camera half, forbidden camera half, eyeline arrows, entrance/exit direction, and any deliberate axis-change bridge. Seedance prompts must repeat the same left/right relationship and forbid axis crossing, eyeline reversal, flipped geography, and reversed exit direction unless the scene explicitly designs a visible neutral-axis transition shot.
- Do not make every scene pure A-roll. Use B-roll, reaction shots, inserts, and empty atmosphere shots when they improve rhythm or continuity: hands on props, wind/rain/dust, landscape/room tone, listener reaction, crowd micro-reaction, prop state, doorway/road/river direction, or silence beats. These shots must have story purpose and reference duties, not decorative filler.
- Main and recurring speaking characters must have a **VOICE_LOCK** in Seedance prompts. Voice is a character asset, but it is controlled by text/audio direction, not by Image2 boards. For each speaking character, define age-range voice, pitch, timbre, breath, volume, rhythm, articulation, accent/dialect level, emotional variation, and forbidden wrong voices. Repeat the relevant voice lock before every spoken line or dialogue segment, then preserve the user's original line verbatim.
...
Storyboard boards must follow `references/storyboard-prompts/director-storyboard-layout-bible.md`: no character turnaround sheets inside the storyboard; use director sketch / previs line-art instead of photoreal scene rendering; include top-down movement floor plan, 180-degree axis/screen-direction lock where applicable, camera setup, motion route, light/atmosphere, blocking, sound/VFX, negative constraints, and a concise shot guide. Shot count and duration are directed by rhythm and emotion, not by a fixed 5-shot or fixed-15s template; a single held shot is valid when it carries the scene better, and a long scene should continue across multiple linked Seedance segments. Dialogue scenes should normally include reaction shots, purposeful inserts, or empty atmosphere beats when they help performance breathe or preserve continuity.
